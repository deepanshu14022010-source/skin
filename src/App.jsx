...
function AkrivoApp() {
  ...
  return (
    <div className="mx-auto min-h-screen w-full max-w-6xl px-4 pb-8 pt-4 sm:px-6">
      ...
      {!user && ["login", "signup", "forgot", "otp"].includes(screen) && <AuthScreen mode={screen} onUser={setUser} onNavigate={next => ["dashboard", "onboarding"].includes(next) ? enterSignedIn(next) : navigate(next)} notify={notify} />}
    ...
    <main className="grid gap-4 lg:grid-cols-[1.2fr_.8fr]">
      <section className="min-h-[440px] overflow-hidden rounded-lg border border-white/70 bg-[linear-gradient(135deg,rgba(255,253,249,.92),rgba(247,216,210,.55))] p-6 shadow-soft sm:p-10">
        ...
        <div className="mt-8 flex flex-col gap-3 sm:flex-row">
          ...
          <button className="btn-secondary" onClick={() => onNavigate("otp")}>Email code</button>
        </div>
      </section>
...
function AuthScreen({ mode, onUser, onNavigate, notify }) {
  ...
  async function submit(event) {
    ...
    try {
      if (mode === "otp" && !otpSent) {
        await api("/api/auth/otp/request", { method: "POST", body: JSON.stringify({ email: form.email }) });
        setOtpSent(true);
        notify("Verification code sent.");
        return;
      }
      if (mode === "otp") {
        const data = await api("/api/auth/otp/verify", { method: "POST", body: JSON.stringify({ email: form.email, otp: form.otp }) });
        onUser(data.user);
      ...
      if (mode === "forgot" && !otpSent) {
        ...
        setOtpSent(true);
        notify("If this email exists, a verification code has been sent.");
        return;
      }
      if (mode === "forgot") {
  ...
  return (
    <main className="mx-auto max-w-md">
      <section className="card">
        <div className="mb-4 grid grid-cols-4 gap-2 rounded-lg bg-[#f4eee6] p-1">
          {["login", "signup", "otp", "forgot"].map(item => (
            <button key={item} className={`min-h-10 rounded-md text-sm font-black ${mode === item ? "bg-white shadow" : "text-[#73716d]"}`} onClick={() => onNavigate(item)}>
              {item === "forgot" ? "Forgot" : item === "otp" ? "OTP" : item[0].toUpperCase() + item.slice(1)}
            </button>
          ))}
        </div>
        <form onSubmit={submit} className="grid gap-4">
          ...
          {mode === "otp" && otpSent && <Field label="Verification code" value={form.otp} onChange={otp => setForm({ ...form, otp: otp.replace(/\D/g, "").slice(0, 6) })} inputMode="numeric" placeholder="6-digit code" />}
          ...
          <button className="btn-primary" disabled={busy}>{busy ? "Please wait..." : mode === "signup" ? "Create account" : mode === "forgot" ? otpSent ? "Reset password" : "Send code" : mode === "otp" ? otpSent ? "Verify code" : "Send code" : "Login"}</button>
