  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.shell}>
        ...
        {screen === "signup" && <Auth mode="signup" setUser={setUser} go={enterSignedIn} notify={notify} />}
  ...
  return (
    <Screen>
      <Card>
        ...
        <Button label="Email code" secondary onPress={() => go("otp")} />
...
function Auth({ mode, setUser, go, notify }) {
  ...
  return (
    <Screen>
      <Card>
        <Text style={styles.h1}>{mode === "signup" ? "Create account" : mode === "forgot" ? "Forgot password" : mode === "otp" ? "Email code" : "Login"}</Text>
...
function Onboarding({ setUser, go, notify }) {
  ...
  return (
    <Screen>
      ...
      <ChoiceGroup label="Budget level" values={budgetOptions} selected={form.budgetLevel} onSelect={budgetLevel => setForm({ ...form, budgetLevel })} />
