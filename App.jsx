...
function SkinBotPanel() {
  const [messages, setMessages] = useState([{ role: "assistant", content: "Ask about routine order, patch testing, product choices, reminders, or what to do if irritation starts." }]);
  ...
  return (
    <section className="card grid gap-4">
      <div className="flex items-center gap-3">
        <Bot className="text-rose" />
        <div>
          <h3 className="text-lg font-black">Skin Bot</h3>
          <p className="muted">Groq-powered when configured, with local safety fallback.</p>
        </div>
      </div>
      <div className="grid max-h-72 gap-2 overflow-y-auto rounded-lg bg-[#fff9f2] p-3">
        {messages.map((item, index) => <p key={`${item.role}-${index}`} className={`rounded-lg p-3 text-sm font-semibold ${item.role === "user" ? "ml-8 bg-[#e7f1eb] text-[#245c52]" : "mr-8 bg-white text-[#554f4b]"}`}>{item.content}</p>)}
      </div>
