...
import { authFromRequest, createSession, destroySession, findOrCreateOtpUser, login, serializeUser, signup } from "../services/authService.js";
import { logSecurity } from "../utils/logger.js";
import { assertBrevoConfigured, sendOtpEmail } from "../services/emailService.js";
import { createEmailOtp, verifyEmailOtp } from "../services/otpService.js";
import { authSchema, forgotPasswordSchema, otpRequestSchema, otpVerifySchema, passwordResetSchema, validate } from "../utils/validation.js";
  ...
  if (userExists) {
    ...
    try {
      ...
      store.transaction(current => {
        delete current.emailOtps[input.email];
      });
      throw error;
    }
    logSecurity("password_reset_otp_requested", { ip: clientIp(req), email: maskEmail(input.email) });
  }
  res.json({ ok: true, message: "If an account exists, a verification code has been sent." });
}));
